...
const ProductsModal = () => {
  ...
  React.useEffect(() => {
    const foundProductId = basket.list.findIndex((item) => item.id === id);
    setIsProductAdded(foundProductId > -1 ? true : false);
    ...
    <Modal
      ...
      {!product.isLoading ? (
        <ModalInner>
          ...
          <Card title={currProduct?.title} style={CardStyles} bordered={false}>
